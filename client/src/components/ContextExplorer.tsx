...
export default function ContextExplorer({
  ...
  const filteredTasks = selectedContextId
    ? tasks.filter(task => task.contextId === selectedContextId)
    : tasks;
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="border-b border-terminal-border px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            >
              <Plus size={16} />
              <span className="text-sm font-mono">New Task</span>
            </button>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {filteredTasks.length === 0 ? (
            <div className="text-center text-terminal-text/30 mt-20">
              <p className="font-mono text-sm">No tasks in this context.</p>
              <p className="font-mono text-xs mt-2">Click "New Task" to add one ↑</p>
            </div>
          ) : (
